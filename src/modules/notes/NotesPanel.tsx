...
export interface NotesPanelProps {
  ...
  theme?: "light" | "dark";
  /** Si Life OS controla la ruta, pásale la página activa; si no, gestiona estado interno */
  activePageId?: string | null;
  onNavigate?: (pageId: string | null) => void;
}

/**
 * Pantalla "Notas" completa para la sección Productividad de Life OS.
 * Móvil: lista <-> editor en dos vistas. Escritorio: lado a lado.
 * Incluye buscador, favoritos, papelera y drag&drop de reorganización.
 */
export function NotesPanel({ supabase, theme = "dark", activePageId, onNavigate }: NotesPanelProps) {
  ...
  const pageId = activePageId !== undefined ? activePageId : internalId;
  const setPageId = (id: string | null) => (onNavigate ? onNavigate(id) : setInternalId(id));
  ...
  if (loading) return <div className="lifeos-notes-empty">Cargando notas…</div>;
  if (error) return <div className="lifeos-notes-empty">Error: {error}</div>;

  return (
    <div className={"lifeos-notes" + (pageId ? " lifeos-notes--editing" : "")}>
      <aside className="lifeos-notes__sidebar">
        ...
        {showTrash ? (
          ...
          <div>
            ...
            {results.map((r) => (
              ...
              >
                <button className="lifeos-notes-tree__label" onClick={() => setPageId(r.id)} title={r.title}>
            ...
            {favorites.length > 0 && (
              <div>
                ...
                {favorites.map((f) => (
                  ...
                  >
                    <button className="lifeos-notes-tree__label" onClick={() => setPageId(f.id)} title={f.title}>
      ...
      <main className="lifeos-notes__main">
        {pageId && (
          <button className="lifeos-notes__back" onClick={() => setPageId(null)}>
            ← Notas
          </button>
        )}
